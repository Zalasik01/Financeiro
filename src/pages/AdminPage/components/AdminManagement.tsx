...
// Firebase imports removidos - migrado para Supabase
import { Database, ShieldOff, Store, UserPlus, Users } from "lucide-react";
...
export const AdminManagement: React.FC<AdminManagementProps> = ({
  ...
  const handleCreateAdmin = async (e: FormEvent) => {
    ...
    try {
      const createAdminUserFunction = httpsCallable(
        functions,
        "createAdminUser"
      );
          ...
          {adminUsers.length === 0 ? (
            ...
            <ul className="space-y-2">
              {adminUsers.map((admin) => (
                <li
                  key={admin.uid}
                  className="p-3 border rounded-md flex justify-between items-center bg-white dark:bg-slate-800"
                >
                  <div>
                  ...
                  {currentUser?.uid !== admin.uid && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => onSetUserToRevoke(admin)}
                    >
