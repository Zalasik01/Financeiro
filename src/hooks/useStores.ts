...
import { Store, StoreClosing, PaymentMethod, MovementType, MovementItem, DREData, StoreMeta, StoreRanking } from '@/types/store';
...
const defaultMovementTypes: MovementType[] = [
  {
    id: '1',
    name: 'Vendas',
    category: 'entrada',
...
export const useStores = () => {
  ...
  const [movementTypes, setMovementTypes] = useState<MovementType[]>(defaultMovementTypes);
  ...
  const updatePaymentMethod = (id: string, methodUpdates: Partial<PaymentMethod>) => {
  ...
  const updateMovementType = (id: string, typeUpdates: Partial<MovementType>) => {
    setMovementTypes(prev => 
      prev.map(type => 
        type.id === id ? { ...type, ...typeUpdates } : type
      )
    );
  };
  ...
  const addStoreClosing = (closing: Omit<StoreClosing, 'id' | 'createdAt' | 'totalEntradas' | 'totalSaidas' | 'totalOutros' | 'netResult'>) => {
    const { totalEntradas, totalSaidas, totalOutros } = calculateTotals(closing.movements);
    ...
    setClosings(prev => 
      prev.map(closing => {
        ...
        if (closingUpdates.movements) {
          ...
          const finalBalance = closingUpdates.finalBalance !== undefined ? closingUpdates.finalBalance : closing.finalBalance;
          const initialBalance = closingUpdates.initialBalance !== undefined ? closingUpdates.initialBalance : closing.initialBalance;
  ...
  const getClosingsWithDetails = () => {
    return closings.map(closing => ({
      ...
      movements: closing.movements.map(movement => ({
        ...movement,
        movementType: movementTypes.find(type => type.id === movement.movementTypeId),
        paymentMethod: paymentMethods.find(method => method.id === movement.paymentMethodId),
      })),
    }));
  };

  const generateDRE = (startDate: Date, endDate: Date, storeId?: string): DREData => {
    ...
    const storeResults = filteredStores.map(store => {
      const storeClosings = filteredClosings.filter(closing => closing.storeId === store.id);
      const totalReceitas = storeClosings.reduce((sum, closing) => sum + closing.totalEntradas, 0);
      const totalDespesas = storeClosings.reduce((sum, closing) => sum + closing.totalSaidas, 0);
    ...
    const consolidated = {
      totalReceitas: storeResults.reduce((sum, result) => sum + result.totalReceitas, 0),
      totalDespesas: storeResults.reduce((sum, result) => sum + result.totalDespesas, 0),
      resultadoLiquido: storeResults.reduce((sum, result) => sum + result.resultadoLiquido, 0),
    };
  ...
  const updateGoal = (id: string, goalUpdates: Partial<StoreMeta>) => {
    setGoals(prev => 
      prev.map(goal => 
        goal.id === id ? { ...goal, ...goalUpdates } : goal
      )
    );
  };
  ...
  const getStoreRankings = (): StoreRanking[] => {
    return stores.map(store => {
      ...
      const averageBalance = totalClosings > 0 ? (totalInitialBalance + totalFinalBalance) / (totalClosings * 2) : 0;
      
      const sortedClosings = [...storeClosings].sort((a, b) => 
      ...
      return {
        ...
        totalExpenses,
        averageBalance,
        lastClosingDate: sortedClosings.length > 0 ? new Date(sortedClosings[0].closingDate) : undefined
      };
    });
  };
