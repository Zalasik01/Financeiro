...
import { ref, onValue, push, set, remove, update, query, orderByChild, serverTimestamp, get, equalTo } from "firebase/database"; // Adicionado get e equalTo
...
export const useStores = () => {
    ...
    const unsubscribe = onValue(closingsQuery, (snapshot) => {
      ...
      if (data) {
        const list: StoreClosing[] = Object.keys(data).map(key => ({
          ...
          movements: data[key].movements ? Object.values(data[key].movements) : []
        }));
        setClosings(list);
      } else {
        setClosings([]);
  ...
  const addStore = async (storeData: Omit<Store, "id" | "createdAt">) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para adicionar uma loja.", variant: "destructive" });
      return null;
    }
    try {
      const storesNodeRef = ref(db, `users/${currentUser.uid}/appStores`);
      const newStoreRef = push(storesNodeRef);
      const storeToSave = { ...storeData, createdAt: serverTimestamp() };
      ...
      const errorMessage = (error as Error).message || "Não foi possível adicionar a loja.";
  ...
  const updateStore = async (id: string, storeUpdates: Partial<Omit<Store, "id" | "createdAt">>) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para atualizar uma loja.", variant: "destructive" });
      return;
    }
    try {
      ...
      const errorMessage = (error as Error).message || "Não foi possível atualizar a loja.";
  ...
  const deleteStore = async (id: string) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para deletar uma loja.", variant: "destructive" });
      return;
    }
    try {
      const storeRef = ref(db, `users/${currentUser.uid}/appStores/${id}`);
      ...
      const closingsQuery = query(ref(db, `users/${currentUser.uid}/appClosings`), orderByChild("storeId"), equalTo(id));
      onValue(closingsQuery, (snapshot) => {
        snapshot.forEach((childSnapshot) => {
          remove(childSnapshot.ref);
        });
      }, { onlyOnce: true });

      const goalsQuery = query(ref(db, `users/${currentUser.uid}/appGoals`), orderByChild("storeId"), equalTo(id));
      onValue(goalsQuery, (snapshot) => {
        snapshot.forEach((childSnapshot) => {
          remove(childSnapshot.ref);
        });
      }, { onlyOnce: true });
      ...
      const errorMessage = (error as Error).message || "Não foi possível deletar a loja.";
  ...
  const addPaymentMethod = async (
    ...
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para adicionar um método de pagamento.", variant: "destructive" });
      return null;
    }
    try {
      const methodsNodeRef = ref(db, `users/${currentUser.uid}/appPaymentMethods`);
      ...
      toast({ title: "Sucesso!", description: "Método de pagamento adicionado." });
      return { ...methodData, id: newMethodRef.key!, createdAt: new Date() };
    } catch (error) {
      const errorMessage = (error as Error).message || "Não foi possível adicionar o método de pagamento.";
  ...
  const updatePaymentMethod = async (
    ...
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para atualizar um método de pagamento.", variant: "destructive" });
      return;
    }
    try {
      const methodRef = ref(db, `users/${currentUser.uid}/appPaymentMethods/${id}`);
      await update(methodRef, methodUpdates);
      toast({ title: "Sucesso!", description: "Método de pagamento atualizado." });
    } catch (error) {
      const errorMessage = (error as Error).message || "Não foi possível atualizar o método de pagamento.";
  ...
  const deletePaymentMethod = async (id: string) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para deletar um método de pagamento.", variant: "destructive" });
      return;
    }
    try {
      const methodRef = ref(db, `users/${currentUser.uid}/appPaymentMethods/${id}`);
      await remove(methodRef);
      toast({ title: "Sucesso!", description: "Método de pagamento deletado." });
    } catch (error) {
      const errorMessage = (error as Error).message || "Não foi possível deletar o método de pagamento.";
  ...
  const addMovementType = async (typeData: Omit<MovementType, "id" | "createdAt">) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para adicionar um tipo de movimento.", variant: "destructive" });
      return null;
    }
    try {
      ...
      toast({ title: "Sucesso!", description: "Tipo de movimento adicionado." });
      return { ...typeData, id: newTypeRef.key!, createdAt: new Date() };
    } catch (error) {
      const errorMessage = (error as Error).message || "Não foi possível adicionar o tipo de movimento.";
  ...
  const updateMovementType = async (
    ...
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para atualizar um tipo de movimento.", variant: "destructive" });
      return;
    }
    try {
      const typeRef = ref(db, `users/${currentUser.uid}/appMovementTypes/${id}`);
      await update(typeRef, typeUpdates);
      toast({ title: "Sucesso!", description: "Tipo de movimento atualizado." });
    } catch (error) {
      const errorMessage = (error as Error).message || "Não foi possível atualizar o tipo de movimento.";
  ...
  const deleteMovementType = async (id: string) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para deletar um tipo de movimento.", variant: "destructive" });
      return;
    }
    try {
      const typeRef = ref(db, `users/${currentUser.uid}/appMovementTypes/${id}`);
      ...
      const errorMessage = (error as Error).message || "Não foi possível deletar o tipo de movimento.";
  ...
  const calculateTotals = useCallback((movements: MovementItem[]) => {
  ...
  const addStoreClosing = async (
    closingData: Omit< // Renomeado parâmetro para consistência interna
      StoreClosing,
      "id" | "createdAt" | "totalEntradas" | "totalSaidas" | "totalOutros" | "netResult"
    >
  ) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para adicionar um fechamento.", variant: "destructive" });
      return null;
    }
    try {
      ...
      const movementsToSave = (closingData.movements || []).map((m: MovementItem) => ({ // Adicionada tipagem para m
        ...m,
        // Se movementId não existir, pode ser gerado aqui ou assumir que já existe
        id: m.id || `${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
      ...
      const closingToSave = {
        ...
        totalSaidas,
        totalOutros,
        netResult: (closingData.finalBalance || 0) - (closingData.initialBalance || 0), // Adicionado fallback para 0
      ...
      return {
        ...
        totalEntradas, totalSaidas, totalOutros,
        netResult: (closingData.finalBalance || 0) - (closingData.initialBalance || 0), // Adicionado fallback para 0
        createdAt: new Date()
      };
    } catch (error) {
      const errorMessage = (error as Error).message || "Não foi possível adicionar o fechamento.";
  ...
  const updateClosing = async (id: string, closingUpdates: Partial<Omit<StoreClosing, "id" | "createdAt">>) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para atualizar um fechamento.", variant: "destructive" });
      return;
    }
    try {
      ...
      if (closingUpdates.closingDate) {
        (updatesToSave as any).closingDate = closingUpdates.closingDate.toISOString();
      }
      ...
      if (closingUpdates.movements) {
        ...
        if (closingUpdates.finalBalance === undefined || closingUpdates.initialBalance === undefined) {
      ...
      const errorMessage = (error as Error).message || "Não foi possível atualizar o fechamento.";
  ...
  const deleteClosing = async (id: string) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para deletar um fechamento.", variant: "destructive" });
      return;
    }
    try {
      const closingRef = ref(db, `users/${currentUser.uid}/appClosings/${id}`);
      ...
      const errorMessage = (error as Error).message || "Não foi possível deletar o fechamento.";
  ...
  const closingsWithDetails = useMemo(() => {
    return closings.map((closing) => ({
      ...
      movements: (closing.movements || []).map((movement: MovementItem) => ({ // Garante que movements seja um array e tipa movement
        ...movement,
  ...
  const addGoal = async (goalData: Omit<StoreMeta, "id" | "createdAt">) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para adicionar uma meta.", variant: "destructive" });
      return null;
    }
    try {
      ...
      const errorMessage = (error as Error).message || "Não foi possível adicionar a meta.";
  ...
  const updateGoal = async (id: string, goalUpdates: Partial<Omit<StoreMeta, "id" | "createdAt">>) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para atualizar uma meta.", variant: "destructive" });
      return;
    }
    try {
      ...
      const errorMessage = (error as Error).message || "Não foi possível atualizar a meta.";
  ...
  const deleteGoal = async (id: string) => {
    if (!currentUser) {
      toast({ title: "Erro!", description: "Você precisa estar logado para deletar uma meta.", variant: "destructive" });
      return;
    }
    try {
      const goalRef = ref(db, `users/${currentUser.uid}/appGoals/${id}`);
      ...
      const errorMessage = (error as Error).message || "Não foi possível deletar a meta.";
