...
export const useClientesFornecedores = () => {
  ...
  useEffect(() => {
    ...
    const consultaClientesFornecedores = query(
      ref(db, caminhoBase),
      orderByChild("nome")
    );
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        ...
        if (dados) {
          const lista: ClienteFornecedor[] = Object.keys(dados).map(
            (chave) => ({
              id: chave,
              ...dados[chave],
      ...
      (erro) => {
        ...
        toast({
          ...
          description:
            "Não foi possível carregar a lista de clientes/fornecedores.",
          variant: "destructive",
        });
        setCarregando(false);
      }
    ...
    async (
      ...
      if (!currentUser || !selectedBaseId) {
        toast({
          ...
          description:
            "Usuário não autenticado ou base de dados não selecionada.",
          variant: "destructive",
        });
        return null;
      }
      try {
        ...
        toast({
          ...
          description: `${dados.ehFornecedor ? "Fornecedor" : "Cliente"} "${
            dados.nome
        ...
        toast({
          ...
          description:
            "Não foi possível adicionar o cliente/fornecedor. Tente novamente.",
          variant: "destructive",
        });
        return null;
      }
  ...
  const atualizarClienteFornecedor = useCallback(
    async (
      id: string,
      dados: Partial<Omit<ClienteFornecedor, "id" | "dataCadastro">>
    ) => {
      if (!currentUser || !selectedBaseId) {
        toast({
          title: "Erro de Autenticação",
  ...
  const deletarClienteFornecedor = useCallback(
    async (id: string, nome: string) => {
      ...
      try {
        ...
        // Corrigido: Adicionado parêntese de abertura
        console.error("Erro ao deletar cliente/fornecedor:", erro);
        toast({
          title: "Falha na Remoção",
          description:
            "Não foi possível remover o cliente/fornecedor. Tente novamente.",
          variant: "destructive",
        });
        return false;
