...
const defaultCategories: Category[] = [
  ...
  { id: '4', name: 'Alimentação', type: 'expense', color: '#F59E0B', icon: '🍔' },
  { id: '5', name: 'Transporte', type: 'expense', color: '#8B5CF6', icon: '🚗' },
];
...
export const useFinance = () => {
  ...
  const updateTransaction = (id: string, transactionData: Partial<Transaction> | null) => {
    ...
    setTransactions(prev =>
      prev.map(transaction => {
        ...
        if (transactionData.categoryId && transactionData.categoryId !== transaction.categoryId) {
          updatedTransaction.category = categories.find(cat => cat.id === transactionData.categoryId);
        }
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = `finances_backup_${new Date().toISOString().slice(0, 10)}.json`;
