...
import {
  ...
  maskPhone,
  onlyNumbers,
} from "@/utils/formatters"; // Importa funções de máscara
import { Building, Loader2, User } from "lucide-react"; // Ícone de carregamento e para tipo de pessoa
...
interface ClienteFornecedorFormProps {
  ...
  aoSubmeter: (
    dados: Omit<ClienteFornecedor, "id" | "dataCadastro" | "dataAtualizacao">
  ) => Promise<string | null | void>; // Função chamada ao submeter
...
export const ClienteFornecedorForm: React.FC<ClienteFornecedorFormProps> = ({
  ...
  const [formData, setFormData] = useState<
    Omit<ClienteFornecedor, "id" | "dataCadastro" | "dataAtualizacao">
  >({
    nome: "",
  ...
  useEffect(() => {
    if (dadosIniciais) {
      setFormData({
        ...
        nomeFantasia:
          dadosIniciais.tipoDocumento === "CNPJ"
        ...
        cnaeFiscal:
          dadosIniciais.tipoDocumento === "CNPJ"
            ? dadosIniciais.cnaeFiscal || ""
            : "",
        cnaeFiscalDescricao:
          dadosIniciais.tipoDocumento === "CNPJ"
            ? dadosIniciais.cnaeFiscalDescricao || ""
            : "",
        ehCliente:
          dadosIniciais.ehCliente !== undefined
            ? dadosIniciais.ehCliente
            : true,
        ehFornecedor: dadosIniciais.ehFornecedor || false,
        observacoes: dadosIniciais.observacoes || "",
      });
    } else {
      // Resetar para o estado inicial se não houver dados iniciais (ex: ao mudar de edição para novo)
      setFormData({
        nome: "",
  ...
  const manipularMudancaDocumento = (
  ...
  useEffect(() => {
    const buscarCnpjSeValido = async () => {
      if (
        ...
        onlyNumbers(formData.numeroDocumento).length === 14 &&
        !buscandoCNPJ
      ) {
  ...
  const buscarDadosCNPJ = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          erroData.message || `Erro ao buscar CNPJ: ${response.statusText}`
        );
      }
      ...
      setFormData((prev) => ({
        ...
        endereco: {
          logradouro:
            prev.endereco?.logradouro?.trim() === ""
              ? data.logradouro || ""
              : prev.endereco?.logradouro,
          numero:
            prev.endereco?.numero?.trim() === ""
              ? data.numero || ""
              : prev.endereco?.numero,
          complemento:
            prev.endereco?.complemento?.trim() === ""
              ? data.complemento || ""
              : prev.endereco?.complemento,
          bairro:
            prev.endereco?.bairro?.trim() === ""
              ? data.bairro || ""
              : prev.endereco?.bairro,
          cidade:
            prev.endereco?.cidade?.trim() === ""
              ? data.municipio || ""
              : prev.endereco?.cidade,
          estado:
            prev.endereco?.estado?.trim() === ""
              ? data.uf || ""
              : prev.endereco?.estado,
          cep:
            prev.endereco?.cep?.trim() === ""
              ? data.cep
                ? maskCEP(data.cep)
                : ""
              : prev.endereco?.cep,
          descricaoTipoLogradouro:
            prev.endereco?.descricaoTipoLogradouro?.trim() === ""
              ? data.descricao_tipo_de_logradouro || ""
              : prev.endereco?.descricaoTipoLogradouro,
        } as ClienteFornecedor["endereco"], // Força a tipagem correta
        cnaeFiscal:
          prev.cnaeFiscal?.trim() === ""
            ? data.cnae_fiscal?.toString() || ""
            : prev.cnaeFiscal,
        cnaeFiscalDescricao:
          prev.cnaeFiscalDescricao?.trim() === ""
    ...
    <form onSubmit={submeterFormulario} className="space-y-4 p-1 pr-3">
      {!estaEditando && (
        <div className="mb-6 p-4 border-b">
          ...
          <RadioGroup
            value={
              formData.tipoDocumento === "Outro"
                ? "CPF"
                : formData.tipoDocumento
            } // Garante que 'Outro' não seja selecionado aqui
      ...
      <div>
        <Label htmlFor="nome">
          {formData.tipoDocumento === "CNPJ" ? "Razão Social *" : "Nome *"}
        </Label>
        <Input
          id="nome"
          name="nome"
      ...
      <fieldset className="border p-4 rounded-md space-y-3">
        ...
        {formData.tipoDocumento === "CNPJ" && (
          <div>
            ...
            <Input
              id="descricaoTipoLogradouro"
              name="descricaoTipoLogradouro"
              value={formData.endereco?.descricaoTipoLogradouro || ""}
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center space-x-2">
          <Checkbox
            ...
            onCheckedChange={() => manipularMudancaCheckbox("ehCliente")}
            disabled={camposDesabilitados}
          />
          <Label htmlFor="ehCliente" className="font-normal">
            É Cliente
          </Label>
        </div>
        <div className="flex items-center space-x-2">
          <Checkbox
            ...
            onCheckedChange={() => manipularMudancaCheckbox("ehFornecedor")}
            disabled={camposDesabilitados}
          />
          <Label htmlFor="ehFornecedor" className="font-normal">
            É Fornecedor
          </Label>
        </div>
      </div>
      ...
      <div className="flex justify-end space-x-2 pt-4">
        ...
        <Button type="submit" disabled={buscandoCNPJ || camposDesabilitados}>
