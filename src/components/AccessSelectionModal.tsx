...
export const AccessSelectionModal: React.FC<AccessSelectionModalProps> =
  React.memo(
    ({ isOpen, onClose, onSelectBase, bases, isAdmin }) => {
      ...
      useEffect(() => {
        console.log("🔍 [AccessSelectionModal] Estado mudou:", {
          isOpen,
      ...
      useEffect(() => {
        console.log(
          "📊 [AccessSelectionModal] Props tracking - isOpen:",
          isOpen
        );
      }, [isOpen]);
      ...
      useEffect(() => {
        console.log(
          "📊 [AccessSelectionModal] Props tracking - bases length:",
          bases?.length || 0
        );
      }, [bases]);
      ...
      useEffect(() => {
        console.log(
          "📊 [AccessSelectionModal] Props tracking - isAdmin:",
          isAdmin
        );
      }, [isAdmin]);

      const handleSelectBase = (baseId: string) => {
        console.log("🎯 [AccessSelectionModal] Base selecionada:", baseId);
        onSelectBase(baseId);
      };
      ...
      if (!isOpen) {
        console.log("⏸️ [AccessSelectionModal] Modal fechado");
        return null;
      }

      console.log("🎭 [AccessSelectionModal] Renderizando modal:", {
        isOpen,
        basesCount: bases?.length || 0,
    ...
    (prevProps, nextProps) => {
      ...
      const propsChanged =
        ...
        prevProps.onSelectBase !== nextProps.onSelectBase;

      console.log("🔄 [AccessSelectionModal] React.memo comparação:", {
        shouldRerender: propsChanged,
        prevOpen: prevProps.isOpen,
        nextOpen: nextProps.isOpen,
        prevBasesLength: prevProps.bases?.length,
        nextBasesLength: nextProps.bases?.length,
      });

      return !propsChanged; // Return true para NÃO re-renderizar
    }
  );
